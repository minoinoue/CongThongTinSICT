...
public class NewsController extends HttpServlet {
    ...
    private TheLoaiDAO tlDB;
    private TheLoaiTinDAO tltDB;

    @Override
    public void init() throws ServletException {
        nDB = new TinTucDAO();
        tlDB = new TheLoaiDAO();
        tltDB = new TheLoaiTinDAO();
    }
    ...
    protected void manageNews(HttpServletRequest request, HttpServletResponse response)
        ...
        while (attempt < maxRetries) {
            try {
                ...
                listTheLoai = tlDB.getVisibleTheLoaiList();
                listTheLoaiTin = tltDB.getAll();
                break;
            } catch (Exception e) {
                attempt++;
                System.err.println("Attempt " + attempt + " to load data failed: " + e.getMessage());
                if (attempt == maxRetries) {
                    request.setAttribute("error",
                            "Failed to load data after " + maxRetries + " attempts: " + e.getMessage());
                    break;
                }
                nDB = new TinTucDAO();
                tlDB = new TheLoaiDAO();
