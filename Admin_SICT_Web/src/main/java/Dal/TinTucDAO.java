...
public class TinTucDAO extends DBConnect {
        ...
        try (PreparedStatement st = c.prepareStatement(sql)) {
            ...
            while (rs.next()) {
                list.add(new TinTuc(rs.getInt("maTinTuc"), rs.getString("tieuDeTinTuc"),
                        ...
                        rs.getInt("maTheLoaiTin"), rs.getInt("maThanhVien")));
            }
        ...
        try (PreparedStatement st = c.prepareStatement(sql)) {
            ...
            if (rs.next()) {
                return new TinTuc(rs.getInt("maTinTuc"), rs.getString("tieuDeTinTuc"), rs.getString("urlAnh"),
                        ...
                        rs.getInt("maTheLoai"), rs.getInt("maTheLoaiTin"), rs.getInt("maThanhVien"));
            }
        } catch (SQLException e) {
    ...
    public void addTinTuc(TinTuc n, String category) {
        ...
        n.setMaTinTuc(newMaTinTuc);

        String sql = "INSERT INTO TinTuc (MaTinTuc, TieuDeTinTuc, UrlAnh, TrichDanTin, NoiDungTin, NgayCapNhat, SoLanDoc, Tag, MaTheLoai, MaTheLoaiTin, MaThanhVien) "
                + "VALUES (?, ?, ?, ?, ?, GETDATE(), ?, ?, ?, ?, ?)";
        try (PreparedStatement st = c.prepareStatement(sql)) {
    ...
    public void editTinTuc(TinTuc n, int category) {
        ...
        String sql = "UPDATE TinTuc SET TieuDeTinTuc = ?, UrlAnh = ?, TrichDanTin = ?, NoiDungTin = ?, NgayCapNhat = GETDATE(), SoLanDoc = ?, Tag = ?, MaTheLoai = ?, MaTheLoaiTin = ?, MaThanhVien = ? WHERE MaTinTuc = ?";
        try (PreparedStatement st = getConnection().prepareStatement(sql)) {
            ...
            st.setInt(9, n.getMaThanhVien());
            st.setInt(10, n.getMaTinTuc());
            int rowsAffected = st.executeUpdate();
