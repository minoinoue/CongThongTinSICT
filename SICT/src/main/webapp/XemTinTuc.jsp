...
<body>
    ...
    <div class="container my-5">
        <%
            ...
            if (matintucParam != null) {
                try {
                    ...
                    String sql = "SELECT TieuDeTinTuc, NgayCapNhat, NoiDungTin, UrlAnh FROM TinTuc WHERE MaTinTuc = ?";
                    ...
                    <div class="news-detail">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item"><a href="<%= request.getContextPath() %>/TinTuc.jsp">Tin tức</a></li>
        ...
        <h2 class="mb-3 text-primary"><%= tieuDe %></h2>
        <p class="text-muted mb-3"><%= sdf.format(ngayCapNhat) %></p>
        <% if (urlAnh != null && !urlAnh.isEmpty()) { %>
            <img src="<%= urlAnh %>" alt="Ảnh tin tức" class="img-fluid mb-4">
        <% } %>
        <div class="mb-4" style="line-height: 1.8;"><%= noiDung %></div>
        <%
                    } else {
        %>
            <div class="alert alert-warning">Không tìm thấy tin tức.</div>
        <%
                    }
                    rs.close();
                    stmt.close();
        ...
        %>
            <div class="alert alert-warning">Thiếu mã tin tức để hiển thị nội dung.</div>
        <%
            }
        %>
    </div>
